...
class LLMEgoPlanner(AbstractEgoPlanner):
    def plan(self,
        ...
        print("behaviour is {}".format(ego_veh.behaviour))
        obs_list = []
    ...
    def acdc_trajectory_generator(self, 
        ...
        if behaviour == Behaviour.AC:
            ...
            if current_state.acc > 0:
                target_acc = -config["DCC_DEFAULT"]
            else:
                target_acc = current_state.acc -config["DCC_DEFAULT"]
            ...
            if current_state.vel + target_acc*course_t < 0:
                course_t = (0 - current_state.vel)/target_acc
                # target_s = current_state.s + current_state.vel * course_t + 0.5* target_acc * (course_t**2)
                # target_state = State(s=target_s, s_d=0, d=0)
            # else:
            #     target_s = current_state.s + current_state.vel * course_t + 0.5* target_acc * (course_t**2)
            #     target_state = State(s=target_s, s_d=current_state.s_d + target_acc*course_t, d=0)
